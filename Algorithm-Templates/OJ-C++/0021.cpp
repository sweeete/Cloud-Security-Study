#include <iostream> // 引入标准输入输出流库，用于控制台读写 (cin, cout)
...
public:
    ...
    int maxprofit(vector<int>& prices) {
        ...
        for(int i = 1; i < n; i++) {
            int diff = prices[i] - prices[i - 1]; // 计算当前天数相对前一天的价格差值
            if(diff > 0) {         // 贪心策略：只要今天价格高于昨天（存在正向涨幅）
                max_profit+=diff;  // 就将这段正收益累加到总利润中
            }
        }
        return max_profit; // 返回最终累加计算出的最大利润值
    }
};

int main() {
    // 优化 I/O 标准输入输出流性能
    ios_base::sync_with_stdio(false); // 取消 C++ 输入输出流与 C 语言标准流的同步，大幅提高读写性能
    cin.tie(NULL); // 解绑 cin 与 cout，避免每次 cin 输入前强制刷新 cout 输出缓冲区
