...
# ==========================================
# 第二部分：特征工程与模型训练 (2-4 gram)
# ==========================================
def run_experiment():
    # 1. 加载与准备数据
    train_df, val_df, test_df = load_and_preprocess_data('dga_cleaned.txt', 'alexa_cleaned.txt')

    print("\n开始执行 2-4 gram 特征向量化 (混合 2/3/4 位特征)...")
    # ngram_range=(2, 4) 会同时提取 2-gram, 3-gram 和 4-gram
    # min_df=5 用于过滤掉在整个数据集中出现次数少于 5 次的稀疏组合
    ...
    # 3. 训练
    print("模型训练中 (XGBoost 2-4 gram 混合组)...")
    ...
    # 5. 格式化输出 (统一风格)
    print("\n" + "=" * 45)
    print(f"{'实验结果: 二分类 2-4 gram 特征组 (默认参数)':^45}")
    print("=" * 45)
